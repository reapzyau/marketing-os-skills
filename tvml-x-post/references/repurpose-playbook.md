...
Turn one piece of long-form content into 5-10+ social posts. Adapted from Matt Gray's Thread System and Justin Welsh's template recycling approach.
...
| **Quote** | A memorable one-liner or principle | X single tweet |
...
| **Analogy/Metaphor** | A comparison that makes a concept click | X single tweet |
...
- Whether it works best as a single tweet or thread

### Step 3: Generate

For each nugget, generate a post using the matched framework + platform template.

### Step 4: Adapt

Create variants:
- If nugget works as a thread → also create a compressed single tweet version
- If nugget works as a single → consider if it could expand into a thread
- Not every nugget needs both formats — some are better suited to one
...
Long-form source (newsletter, blog, video)
    │
    ├── X Thread (5-8 tweets covering the main argument)
    │
    ├── X Singles (3-5 standalone tweets from individual nuggets)
    │
    └── Additional X Singles (different angles on same nuggets)
```

**From Matt Gray's Thread System:**
```
Thread → carousel → Newsletter topic (8 weeks later) → YouTube video
```

**Key insight:** The thread is the atomic content unit. But for this skill, ANY long-form piece can be the source — not just threads.

---

## Repurpose Rules

1. **Never copy-paste across platforms.** Always adapt for platform culture, format, and length.
2. **One nugget = one post.** Don't cram multiple insights into one social post.
3. **The best nuggets become threads.** If an insight has 3+ sub-points, it's a thread, not a single tweet.
4. **Space out repurposed content.** Don't post 5 versions of the same source on the same day.
5. **Nobody remembers your content like you do.** (Welsh) Repurpose freely — most people didn't see it the first time.
6. **Add new context.** When repurposing, add a personal angle, updated data, or fresh example. Don't just compress.
...
- [ ] Platform formatting is correct (X character count, no hashtags)
